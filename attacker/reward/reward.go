...
func GetRewards(gwEndpoint string, output string) error {
	...
	writer.Write([]string{"Epoch", "Validator Index", "Head", "Target", "Source"})
	...
	for epochNumber <= (latestEpoch - 2) {
		...
		for _, totalReward := range totalRewards {
			writer.Write([]string{strconv.FormatInt(epochNumber, 10), totalReward.ValidatorIndex, totalReward.Head, totalReward.Target, totalReward.Source})
		}
